...
def parse_args(argv):
    p = argparse.ArgumentParser(description="Tactual: control your Mac with hand gestures.")
    ...
    p.add_argument("--hand", choices=["Right", "Left"], help="primary control hand (default Right)")
    ...
    p.add_argument("--anchor", choices=["pinch", "knuckle", "tip"],
                   help="cursor follows the thumb-index pinch point (default), index knuckle, or index tip")
    p.add_argument("--hands", choices=["auto", "one"],
                   help="auto (default): 1 hand, 2 when a second appears (zoom); one: fastest, no zoom")
    p.add_argument("--no-smart-scroll", action="store_true",
                   help="don't redirect scrolls to the nearest scrollable area")
    p.add_argument("--config", type=Path, help="JSON file overriding settings (see fingering/config.py)")
    p.add_argument("--record", type=Path, metavar="FILE.jsonl",
                   help="record hand landmarks (no video) for replay: python -m fingering.record FILE")
...
def other_hands_points(engine: GestureEngine, hands, scale: float = 0.7):
    """Non-primary hands in screen pixels, placed relative to the primary ghost."""
    if engine.hand is None or engine.anchor is None or len(hands) < 2:
        return None
    ax, ay = engine.mapper.map(*engine.anchor, clamp=False)
    cx, cy = engine.cursor if engine.cursor and not engine.prompt else (ax, ay)
    return [[(cx + (x - ax) * scale, cy + (y - ay) * scale)
             for x, y in (engine.mapper.map(px, py, clamp=False) for px, py in h.landmarks)]
            for h in hands if h is not engine.hand]


def overlay_state(engine: GestureEngine) -> str:
    if not engine.enabled:
        return "paused"
    return "pinch" if engine.mode.startswith(("pinch", "drag", "zoom", "scroll", "dial")) else "active"
...
def main(argv=None) -> int:
    ...
    for attr, value in (("camera_index", args.camera), ("primary_hand", args.hand),
                        ("cursor_anchor", args.anchor), ("hands_mode", args.hands)):
        if value is not None:  # CLI flags win over the config file
    ...
    try:
        while True:
            ...
            if overlay:
                overlay.update(overlay_points(engine), overlay_state(engine), engine.prompt,
                               ...
                               other_hands_points(engine, hands))
            if args.no_preview:
                continue
            draw(frame, engine, hands, fps, infer_ms, tracker.two_active)
        ...
        if recorder:
            recorder.close()
            log.info("Recording saved to %s", args.record)
        if overlay:
            overlay.close()
